...
def send_whatsapp_image_message(to, caption, image_url):
    """Sends an image using a URL directly via the Whapi.Cloud API."""
    if not image_url or not image_url.startswith('http'):
        logging.error(f"Invalid image_url provided for send_whatsapp_image_message: {image_url}")
        return False
    ...
    payload = {
        ...
        'view_once': False # Optional: as per user example
    }

    logging.info(f"Attempting to send image by URL directly. Endpoint: {endpoint}, Payload: {json.dumps(payload)}")

    # Call send_whapi_request with is_media=False (or rely on its default for JSON)
    # send_whapi_request handles setting 'Content-Type': 'application/json' when not is_media
    response = send_whapi_request(endpoint, params=payload, method='POST', is_media=False)

    if response and response.get('sent'):
        logging.info(f"Successfully sent image by URL to {to}. Image URL: {image_url}")
        return True
    else:
        logging.error(f"Failed to send image by URL to {to}. Image URL: {image_url}. Response: {response}")
        return False
