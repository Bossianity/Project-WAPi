...
def _handle_show_photos(sender, prop_details, current_language):
    ...
    if not image_urls:
        msg = f"No images are currently available for {prop_name}." if current_language == 'en' else f"لا توجد صور متاحة حالياً لـ {prop_name}."
        send_whatsapp_message(sender, msg)
    else:
        msg = f"Sending {len(image_urls)} image(s) for {prop_name}..." if current_language == 'en' else f"جاري إرسال {len(image_urls)} صورة/صور لـ {prop_name}..."
        send_whatsapp_message(sender, msg)
        time.sleep(0.5)
        for img_data in image_urls:
            send_whatsapp_image_message(sender, img_data['caption'], img_data['url'])
            time.sleep(random.uniform(1.0, 2.0))
    follow_up_text = f"What else would you like to know about {prop_name}? You can ask about prices, booking, or see photos again." if current_language == 'en' else f"ماذا تريد أن تعرف أيضاً عن {prop_name}؟ يمكنك السؤال عن الأسعار، الحجز، أو مشاهدة الصور مرة أخرى."
...
def _handle_show_prices(sender, prop_details, current_language):
    ...
    price_text = f"Prices for {prop_name}:\n- Weekday Night: {weekday_price} SAR\n- Weekend Night: {weekend_price} SAR\n- Monthly Price: {monthly_price} SAR" if current_language == 'en' else f"أسعار {prop_name}:\n- سعر الليلة (أيام الأسبوع): {weekday_price} ريال\n- سعر الليلة (عطلة نهاية الأسبوع): {weekend_price} ريال\n- السعر الشهري: {monthly_price} ريال"
    send_whatsapp_message(sender, price_text)
    follow_up_text = f"What else would you like to know about {prop_name}? You can ask about photos, booking, or see prices again." if current_language == 'en' else f"ماذا تريد أن تعرف أيضاً عن {prop_name}؟ يمكنك السؤال عن الصور، الحجز، أو مشاهدة الأسعار مرة أخرى."
    send_whatsapp_message(sender, follow_up_text)
        ...
        for message in incoming_messages:
            ...
            if user_in_interactive_flow:
                ...
                if interactive_flow_states[sender].get('step') == 'awaiting_booking_creation':
                    ...
                    check_out_time_setting = "11 AM"
                    created_event = create_booking_event(cal_service, calendar_id, prop_id, start_date_obj, num_days, client_name, client_phone, check_in_time_str=check_in_time_setting, check_out_time_str=check_out_time_setting)
                    if created_event:
                        formatted_start_date_msg = start_date_obj.strftime("%B %d, %Y") if booking_lang == 'en' else start_date_obj.strftime("%Y-%m-%d")
                        confirm_msg = f"تم تأكيد حجزك لـ \"{prop_name}\" من تاريخ {formatted_start_date_msg} لمدة {num_days} ليلة/ليالٍ! تسجيل الدخول الساعة 2 مساءً. نتطلع لاستضافتك، {client_name}!" if booking_lang == 'ar' else f"Your booking for \"{prop_name}\" from {formatted_start_date_msg} for {num_days} night(s) is confirmed! Check-in is at 2 PM. We look forward to hosting you, {client_name}!"
                        send_whatsapp_message(sender, confirm_msg)
                        booking_details_text = (f"New Booking Notification:\nProperty ID: {prop_id} ({prop_name})\nClient Name: {client_name}\nClient Phone: {client_phone}\nCheck-in Date: {start_date_obj.strftime('%Y-%m-%d')} (at {check_in_time_setting})\nNumber of Nights: {num_days}\nBooking ID (Calendar Event): {created_event.get('id')}")
                        ...
                        else: logging.warning("OWNER_WHATSAPP_NUMBER not set. Cannot send owner WhatsApp notification.")
                        ...
                        else: logging.warning("LEAD_EMAIL_RECEIVER not set. Cannot send booking email notification.")
                        del interactive_flow_states[sender]
                        logging.info(f"Booking for {prop_id} by {client_name} successful. Event ID: {created_event.get('id')}")
                    else:
                        err_msg = "واجهتنا مشكلة أثناء محاولة تأكيد حجزك لـ \"{prop_name}\". الرجاء التواصل مع الدعم أو المحاولة مرة أخرى قريباً." if booking_lang == 'ar' else f"We encountered an issue while trying to finalize your booking for \"{prop_name}\". Please contact support or try again shortly."
                        send_whatsapp_message(sender, err_msg)
                    ...
                    response_text = "Please make a selection using the buttons or list provided." if current_language == 'en' else "الرجاء تحديد اختيارك باستخدام الأزرار أو القائمة المتوفرة."
            ...
            if llm_response_data['type'] == 'text':
